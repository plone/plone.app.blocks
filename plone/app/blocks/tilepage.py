...
def createTilePage(request, tree):
    ...
    if published is not None and hasattr(published, '__parent__') and \
        hasattr(published.__parent__, '_p_mtime'):
        uniqueXSLName = published.__parent__._p_mtime

    # The URL should include the modification time to make cache invalidation
    # easier, and the original query string, which will be used when the
    # composite page is rendered
    xslURL = "%s/@@blocks-static-content/%s.xsl?%s" % (request.getURL(),
                                                       uniqueXSLName,
                                                       request['QUERY_STRING'])

    stylesheetDeclaration = etree.ProcessingInstruction(
        'xml-stylesheet type="text/xsl" href="%s"' % xslURL)
