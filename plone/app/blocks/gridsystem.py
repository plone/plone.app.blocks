# -*- coding: utf-8 -*-
from plone.app.blocks import utils
from plone.app.blocks.interfaces import IBlocksSettings
from plone.registry.interfaces import IRegistry
from zope.component import queryUtility
from zope.interface import implementer
from zope.interface import Interface

import json
import logging


logger = logging.getLogger('plone.app.blocks')


class IGridSystem(Interface):
    """ Utility to get the grid System
    """


@implementer(IGridSystem)
class BS3GridSystem(object):

    def __init__(self):
        self.offset = 1

    def transform(self, key):
        """ its possible:
            {type: row} -> row
            {type: cell, info: {
                xs:False, sm:False, md:True, lg:true, pos:{x:1 width:10}}} ->
                hidden-xs hidden-sm col-md-10
        """
        element = json.loads(key)
        if 'type' in element and element['type'] == 'row':
            self.offset = 1
            return 'row'
        if 'type' in element and element['type'] == 'cell':
            result = ''
            if 'info' not in element:
                return
            einfo = element['info']
            if 'xs' in einfo and einfo['xs'].lower() == "false":
                result += 'hidden-xs '
            if 'sm' in einfo and einfo['sm'].lower() == "false":
                result += 'hidden-sm '
            if 'md' in einfo and einfo['md'].lower() == "false":
                result += 'hidden-md '
            if 'lg' in einfo and einfo['lg'].lower() == "false":
                result += 'hidden-lg '
            if 'pos' in einfo:
                if einfo['pos']['x'] > self.offset:
                    result += 'col-md-offset-{0:d} '.format(
                        einfo['pos']['x'] - self.offset + 1
                    )
                if 'width' in einfo['pos']:
                    self.offset += einfo['pos']['width']
                    result += 'col-md-{0:d}'.format(einfo['pos']['width'])
            return result


@implementer(IGridSystem)
class DecoGridSystem(object):

    def transform(self, key):
        """ its possible:
            {type: row} -> row
            {type: cell, info: {xs:False, sm:False, md:True, lg:true,
             pos:{x:1 width:10}}} ->
            cell position-1 width-10
        """
        element = json.loads(key)
        if 'type' in element and element['type'] == 'row':
            return 'row'
        elif 'type' in element and element['type'] == 'cell':
            result = 'cell '
            if 'info' in element and 'pos' in element['info']:
                if 'x' in element['info']['pos']:
                    deco_pos = int(element['info']['pos']['x']) - 1
                    result += 'position-{0:d} '.format(deco_pos)
                if 'width' in element['info']['pos']:
                    result += 'width-{0:d}'.format(
                        element['info']['pos']['width']
                    )
            return result


def merge(request, layoutTree):
    """Perform grid system merging for the given page.

    Returns None if the page has no layout.
    """
    # Find layout node
    gridSystem = utils.xpath1(utils.gridXPath, layoutTree)
    if gridSystem is None:
        gridSystem = 'deco'
        registry = queryUtility(IRegistry)
        if registry:
            settings = registry.forInterface(IBlocksSettings, check=False)
            gridSystem = settings.default_grid_system or 'bs3'

    gridUtil = queryUtility(IGridSystem, gridSystem)
    if gridUtil is None:
        logger.warn('Could not apply grid system "{0}"'.format(gridSystem))
        return layoutTree
    gridUtil = gridUtil()
    for layoutGridNode in utils.gridDataXPath(layoutTree):
        gridinfo = layoutGridNode.attrib['data-grid']
        cssGridClass = gridUtil.transform(gridinfo)
        if cssGridClass is not None:
            if 'class' in layoutGridNode.attrib:
                layoutGridNode.attrib['class'] += ' ' + cssGridClass
            else:
                layoutGridNode.attrib['class'] = cssGridClass
        del layoutGridNode.attrib['data-grid']
    return layoutTree
